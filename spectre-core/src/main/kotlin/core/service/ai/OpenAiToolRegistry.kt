...
class OpenAiToolRegistry(
    ...
    private val openAiTools: List<ChatCompletionTool> = toolDefinitions.map { definition ->
        val functionParameters = FunctionParameters.builder()
    ...
    fun requiresConfirm(toolName: String): Boolean {
        return definitionsByName[toolName]?.requiresConfirm ?: false
    }

    fun resolveParameter(toolName: String, argumentsJson: String): String? {
        return definitionsByName[toolName]?.parameterResolver?.invoke(argumentsJson)
    }
    ...
    fun execute(
        ...
        val definition = definitionsByName[toolName]
