...
class DefaultAiService(
    ...
    private fun requestAssistantTurn(context: AiQueryContext): AssistantTurn {
        ...
        val message =
            completion.choices().firstOrNull()?.message()
...
Always prefer safe, bounded diagnostic commands.
            """.trimIndent(),
        )
        return false
    }
...
- If you are unsure whether a command is bounded, do not run it.
            """.trimIndent(),
        )
    }
    ...
    private fun selectSkillWithLlm(
        ...
        val usedTokens =
            response.usage().getOrNull()?.totalTokens()
