...
class AiController(
    ...
    @PreAuthorize("hasPermission(0, T(io.github.vudsen.spectre.api.perm.AppPermissions).LLM_READ)")
    fun currentLLMConfiguration(): LLMConfigurationVO? = aiService.getCurrentLLMConfiguration()
    ...
    fun saveLLMConfiguration(
        @Validated @RequestBody request: UpdateLLMConfigurationDTO,
    ) {
        aiService.updateLLMConfiguration(request)
    }

    @GetMapping("skills")
    fun listSkills(): List<SkillDTO> = aiService.listSkills()
}
