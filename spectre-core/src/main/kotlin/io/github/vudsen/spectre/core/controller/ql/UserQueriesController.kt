...
class UserQueriesController(
    ...
    @QueryMapping
    fun user(): UserQueries = UserQueries

    @SchemaMapping
    fun users(
        @Argument page: Int,
        @Argument size: Int,
    ): PageResult<UserPO> = userService.listUsers(page, size).toPageResult()

    @SchemaMapping
    fun searchByUsername(
        @Argument username: String,
    ): PageResult<UserPO> = userService.searchByUsernamePrefix(username).toPageResult()

    @SchemaMapping
    fun user(
        @Argument id: String,
    ): UserPO? = userService.findById(id.toLong())
}
