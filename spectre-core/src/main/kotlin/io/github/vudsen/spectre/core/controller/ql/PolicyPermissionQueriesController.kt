...
class PolicyPermissionQueriesController(
    ...
    @QueryMapping
    fun permission(): PolicyPermissionQueries = PolicyPermissionQueries

    /**
     * 列出资源下所有的权限
     */
    @SchemaMapping
    fun listPermissionsByResource(
        @Argument(name = "resource") resource: String,
    ): Set<PermissionEntity> = policyPermissionService.findPermissionsByResourceName(resource)
    ...
    fun permissions(
        ...
        policyPermissionService
            .listSubjectPermissions(subjectId.toLong(), subjectType, page, size)
            .toPageResult()

    @SchemaMapping
    fun permission(
        @Argument id: Long,
    ): PolicyPermissionDTO? = policyPermissionService.findById(id)
}
