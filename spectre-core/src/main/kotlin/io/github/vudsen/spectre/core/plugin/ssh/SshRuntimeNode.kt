...
open class SshRuntimeNode :
    AbstractShellRuntimeNode(),
    CloseableRuntimeNode {
    ...
    class InMemoryIOResource(
        keyPairs: String,
    ) : AbstractIoResource<String>(String::class.java, keyPairs) {
        override fun openInputStream(): InputStream? = ByteArrayInputStream(resourceValue.toByteArray(StandardCharsets.UTF_8))
    }
    ...
    companion object {
        ...
        private fun loadOpenSshKey(
            ...
            try {
                ByteArrayInputStream(privateKeyPem.toByteArray(StandardCharsets.UTF_8)).use { bis ->
                    val kps =
                        OpenSSHKeyPairResourceParser.INSTANCE.loadKeyPairs(
                            session,
                            InMemoryIOResource(privateKeyPem),
                            if (secretKeyPassword == null) {
                                FilePasswordProvider.of(secretKeyPassword)
                            } else {
        ...
        fun loadKeyPairAllowEmptyPublic(
            privateKeyPem: String,
            secretKeyPassword: String?,
            session: SessionContext,
        ): KeyPair {
    ...
    ): CommandExecuteResult = execute("sh -c '${commands.joinToString(" ")} | grep $search'")
    ...
    private class SshInteractiveShell(
        ...
        override fun getInputStream(): InputStream = actualIn

        override fun getWriter(): Writer = myWriter

        override fun getOutputStream(): OutputStream = actualOut

        override fun isAlive(): Boolean = !channel.isClosed

        override fun exitCode(): Int? = channel.exitStatus
    ...
    override fun getHomePath(): String = nodeConfig.spectreHome

    override fun isAlive(): Boolean = !session.isClosed
    ...
    override fun getConfiguration(): SshRuntimeNodeConfig = nodeConfig
