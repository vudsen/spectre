...
class DefaultToolchainService(
    ...
    override fun listToolchainItems(
        ...
        page: Int,
        size: Int,
    ): Page<ToolchainItemPO> = toolchainItemRepository.findByIdTypeOrderByIdTagDesc(type, PageRequest.of(page, size))

    override fun listToolchainBundles(
        page: Int,
        size: Int,
    ): Page<ToolchainBundlePO> = toolchainBundleRepository.findAll(PageRequest.of(page, size))

    private fun findToolchainItemAndRequireNonnull(id: ToolchainItemId) =
        toolchainItemRepository
            .findById(id)
            .getOrNull()
    ...
    override fun createToolchainBundle(dto: CreateToolchainBundleDTO) {
        toolchainBundleRepository.save(
            ToolchainBundlePO(
                name = dto.name,
    ...
    override fun isPackageCached(
        ...
        tag: String,
        isArm: Boolean,
    ): Boolean = LocalPackageManager.isCached(type, tag, isArm)
    ...
    override fun findToolchainItemById(
        type: ToolchainType,
        tag: String,
    ): ToolchainItemDTO? = toolchainItemRepository.findById(ToolchainItemId(type, tag)).getOrNull()?.toDTO()
