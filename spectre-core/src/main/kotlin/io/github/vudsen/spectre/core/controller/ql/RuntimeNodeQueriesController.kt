...
class RuntimeNodeQueriesController(
    ...
    @QueryMapping
    fun runtimeNode(): RuntimeNodeQueries = RuntimeNodeQueries

    @SchemaMapping
    fun runtimeNodes(
        @Argument page: Int,
        @Argument size: Int,
    ): PageResult<RuntimeNodeDTO> {
    ...
    private fun mapPluginToVo(ext: RuntimeNodeExtensionPoint): RuntimeNodePluginVO =
        RuntimeNodePluginVO(ext.getId(), ext.name, ext.getDescription(), ext.getConfigurationForm(null))

    @SchemaMapping
    fun plugin(
        @Argument pluginId: String,
    ): RuntimeNodePluginVO? = mapPluginToVo(service.findPluginById(pluginId))
