...
class SecurityConfiguration {
    @Bean
    fun emptyEncryptor(): SecretEncryptor = EmptySecretEncryptor()

    class AesGcmCondition : Condition {
        override fun matches(
            context: ConditionContext,
            metadata: AnnotatedTypeMetadata,
        ): Boolean = SpectreEnvironment.ENCRYPTOR_KEY != null
    }
    ...
    @Bean
    fun secretEncryptorManager(
        encryptors: List<SecretEncryptor>,
        cacheMananager: CacheManager,
    ...
    fun filterChain(
        http: HttpSecurity,
        @Value("\${spring.graphql.http.path:'/graphql'}") graphqlEndpoint: String,
    ): SecurityFilterChain {
        http
            .authorizeHttpRequests { authorize ->
                authorize
                    .requestMatchers("/spectre-api/auth/**")
                    .permitAll()
                    .requestMatchers("/spectre-api/error")
                    .permitAll()
                    .requestMatchers("/spectre/**")
                    .permitAll()
                    .requestMatchers("/")
                    .permitAll()
                    .anyRequest()
