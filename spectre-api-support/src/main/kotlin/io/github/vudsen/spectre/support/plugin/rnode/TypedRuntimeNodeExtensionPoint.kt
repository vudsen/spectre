 ...
 */
abstract class TypedRuntimeNodeExtensionPoint<T : RuntimeNodeConfig, R : RuntimeNode>(
    name: String,
) : RuntimeNodeExtensionPoint(name) {
    final override fun getConfigurationForm(oldConfiguration: RuntimeNodeConfig?): PageDescriptor =
        getConfigurationForm0(oldConfiguration as T?)

    final override fun getViewPage(
        runtimeNodeDTO: RuntimeNodeDTO,
        configuration: RuntimeNodeConfig,
    ): PageDescriptor = getViewPage0(runtimeNodeDTO, configuration as T)

    final override fun createRuntimeNode(config: RuntimeNodeConfig): RuntimeNode = createRuntimeNode0(config as T)
    ...
    ): RuntimeNodeConfig = fillSensitiveConfiguration0(updated as T, base as T)

    final override fun createAttachHandler(
        runtimeNode: RuntimeNode,
        jvm: Jvm,
        bundles: ToolchainBundleDTO,
    ): JvmAttachHandler = createAttachHandler0(runtimeNode as R, jvm, bundles)
    ...
    protected abstract fun fillSensitiveConfiguration0(
        updated: T,
        base: T,
    ): RuntimeNodeConfig

    protected abstract fun getViewPage0(
        runtimeNodeDTO: RuntimeNodeDTO,
        configuration: T,
    ): PageDescriptor
